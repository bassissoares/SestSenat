...
export function buildRanking(facts: Fact[], key: FilterKey, label?: (fact: Fact) => string): RankingRow[] {
  ...
  return [...groups.entries()].map(([rankingKey, group]) => ({ key: rankingKey, label: group.label, quantity: group.quantity, share: total ? group.quantity / total : 0, evolution: years.length > 1 && group.first ? (group.last - group.first) / group.first : null, relatedUnits: [...group.relatedUnits].sort((a, b) => a.localeCompare(b, "pt-BR", { numeric: true })) })).sort((a, b) => b.quantity - a.quantity);
}
