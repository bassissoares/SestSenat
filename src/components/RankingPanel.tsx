...
export function RankingPanel() {
  ...
  return <section className="ranking-section" id="secao-3" aria-labelledby="ranking-title">
    ...
    <div className="table-scroll"><table><caption className="sr-only">Ranking de {dimension.label.toLocaleLowerCase("pt-BR")}</caption><thead><tr><th>Posição</th><th>{dimension.item}</th>{dimension.key === "responsibleName" && <th>Unidade vinculada</th>}<th><button onClick={() => setAscending((value) => !value)}>Quantidade {ascending ? "↑" : "↓"}</button></th><th>Participação</th><th>Variação vs. média anterior</th><th>Ação</th></tr></thead><tbody>{rows.map((row) => <tr key={row.key}><td><strong>#{ranking.indexOf(row) + 1}</strong></td><td>{row.label}</td>{dimension.key === "responsibleName" && <td className="unit-links">{row.relatedUnits.join(" · ")}</td>}<td>{number.format(row.quantity)}</td><td><span className="share-bar"><i style={{ width: `${Math.max(2, row.share * 100)}%` }} /></span>{percent.format(row.share)}</td><td className={row.variation !== null && row.variation < 0 ? "negative" : "positive"}>{row.variation === null ? "Sem histórico" : percent.format(row.variation)}</td><td><button className="row-action" onClick={() => select(row)}>Analisar</button></td></tr>)}</tbody></table></div>
